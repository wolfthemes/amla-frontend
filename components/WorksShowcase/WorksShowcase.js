...
import { Heading, WorkItemMedia } from '../../components';
...
export default function WorksShowcase({ works = [], viewAllHref = '/projets' }) {
	...
	return (
		<section className={cx('component')}>
									...
									<div className={cx('item-overlay')}>
										<div className={cx('item-text')}>
											{work.workTypes?.nodes?.[0]?.name && (
												<span className={cx('item-category')}>{work.workTypes.nodes[0].name}</span>
											)}
											<div className={cx('item-line')}>
												...
												{work.workCompletion && (
													<span className={cx('item-year')}>{work.workCompletion}</span>
												)}
											</div>
										</div>
									</div>
			...
			<Link href={viewAllHref} className={cx('view-all')} aria-label="Voir tous les projets">
